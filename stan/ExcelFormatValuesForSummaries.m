function [ forExcel ] = ExcelFormatValuesForSummaries( PIValuesPage )
    ...
    while status 
       ...
       if status
           ...
           if ~isempty(tagValues)
            ...
            values_key_val = values_key_enum.Current.Value;
            len = tagValues.Count;
            if count == 1 
            ...
            while values_stat 
               if values_stat
                   if ~isempty(values_key_val)
                       ...
                       AFValuesStatus = 1;
                       AFValuesEnum = values_key_val.GetEnumerator();
                       AFValuesCounter = 1;
                       while AFValuesStatus
                           if ~isempty(AFValuesEnum)
                               AFValuesCounter = AFValuesCounter + 1;
                               AFValuesStatus = AFValuesEnum.MoveNext;
                               AFValuesValue = AFValuesEnum.Current;
                               if ~isempty(AFValuesValue)
                                   if isnumeric(AFValuesValue.Value)
                                       tagvalue = num2str(AFValuesValue.Value);
                                   else
                                       tagvalue = char(AFValuesValue.Value.ToString());
                                   end
                                   timestmp = char(AFValuesValue.Timestamp.LocalTime.ToString());
                                   if count ==1 
                                       values_list(AFValuesCounter,1:2) = {timestmp, tagvalue}; 
                                   else
                                       values_list(AFValuesCounter) = {tagvalue};
                                   end
                               end
                           end
                       end
